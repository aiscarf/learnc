...
static void ws_connect_shake_hand(uv_stream_t* stream, unsigned char* data, int data_len) {
...
static void ws_send_data(uv_stream_t* stream, unsigned char* data, int len) {
	int head_size = 2;
	...
	else if (len > 125 && len < 65536)
	{
		...
		data_buf[5] = (len & 0xff000000) >> 24;
		data_buf[4] = (len & 0x00ff0000) >> 16;
		data_buf[3] = (len & 0x0000ff00) >> 8;
		data_buf[2] = (len & 0x000000ff);

		data_buf[9] = (len & 0xff000000) >> 24;
		data_buf[8] = (len & 0x00ff0000) >> 16;
		data_buf[7] = (len & 0x0000ff00) >> 8;
		data_buf[6] = (len & 0x000000ff);
	}
...
static void ws_on_recv_data(uv_stream_t* stream, unsigned char* data, unsigned int len) {
	...
	int head_size = 2;
	if (data_len == 126) // 后面两个字节表示的是数据长度:data[2], data[3],但高位存放低地址
	{
		data_len = data[3] | (data[2] << 8);
		head_size += 2;
	}
	else if (data_len == 127) { // 后面8个字节表示数据长度 2,3,4,5 | 6,7,8,9 但高位存放低地址
	...
	// 发送数据
	static char* test_send_data;
	test_send_data = malloc(65530);
	memset(test_send_data, 'a', 65530);
	test_send_data[0] = 'a';
	test_send_data[1] = 'b';
	test_send_data[2] = 'c';
	test_send_data[3] = 'd';
	ws_send_data(stream, test_send_data, 65530);
	// end
}
...
static void uv_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) {
	...
	// 如果客户端主动关闭 0x88 状态码
	if ((unsigned char) (buf->base[0] == 0x88)) // 关闭 状态码
	{
		printf("ws closing!!!\n");
		return;
	}
	...
	// 暂不考虑粘包这些问题
	ws_on_recv_data(stream, buf->base, 0);
	// end
}
